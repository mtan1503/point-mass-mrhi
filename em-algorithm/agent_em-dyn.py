#!/usr/bin/env python3
'''from point-mass-mrhi/ folder run: ./em-algorithm/agent_em-dyn.py
...
def m_step(r_ik, x, k_agent, k_env):
    ...
    pi_k = np.sum(r_ik,axis=1) / Nk[k_agent]
    remainder = 1 - pi_k[k_agent,:]
    sumN_rik = np.sum(r_ik,axis=1)
    scale = remainder / (np.sum(sumN_rik[k_env],axis=0))
    scale = np.nan_to_num(scale)
    pi_k[k_env,:] = sumN_rik[k_env]*scale
...
''' Parameters for each mass:
    ...
    '''
from time_param import h,T,N,t_p,delta_N,steps,N_t,trials
...
for t in range(trials):
    ...
    for i in steps:
        ...
        for j in range(max_iter):
            ...
            # M: maximization step
            mu, cov_temp, pi = m_step(r_i[:,i-delta_N:i+1,:,t], data[i-delta_N:i+1,:,:],k_agent,k_env)
        ...
        if i>10:
            ...
            k_agent = i_k[mask]                     # remove from agency options
            # while more than one possible agency cluster
            while sum(mask)>1:
                max_cov = k_agent[np.argmax(cov[k_agent,0,0])]  # maximum covariance
                mask[max_cov] = False   # no-agency cluster
                k_agent = i_k[mask]     # remove max cov from agency k
                if sum(mask)==1: break
                max_eps = k_agent[np.argmax(mu[k_agent,1])] # maximum pred error
                mask[max_eps] = False   # no-agency cluster
                k_agent = i_k[mask]     # remove max error from agency k
            k_agent = k_agent[0]        # choose integer
            k_env = i_k[~mask]          # no-agency clusters are opposite of agency cluster

        # agency parameter, B_hat, with mu and r_i for x_hat and dx_hat
        B_hat[i,:,t] = np.dot(pi[[k_agent],:].T,mu[k_agent,[0]])
        ...
        t_agency += h*r_i[k_agent,i,:,t]     # time that SoA
        t_agent_1trial += h*r_i[k_agent,i,:,t]
        mu_agency[i,:,t] = mu[k_agent,:]
        cov_agency[i,t] = cov[k_agent,0,0]
        pi_agency[i,:,t] = pi[k_agent,:]
        soa[i,:,t] = r_i[k_agent,i,:,t]
        eps_dx_agency += np.sum(np.abs(eps_dx[i,k_agent,t]))
...
clusters[n_n+k_agent] = str(k_agent)+' (agency)'
clusters_init[n_n+0] = str(0)+' (agency)'
for i in range(n_n):
    clusters[i] = '$j=%s$'%(i)
    clusters_init[i] = '$j=%s$'%(i)
for i in range(len(k_env)):
    clusters[n_n+k_env[i]] = str(k_env[i])+' (no agency)'
    clusters_init[n_n+i+1] = str(i+1)+' (no agency)'
k_clusters = np.arange(n_k)
k_clusters[k_agent] = 0
k_clusters[k_env] = np.arange(1,n_k)
...
for i in range(n_k):
    ...
    plt.xlabel('Time /s')
    for j in range(n_n):
        plt.scatter(t_p,r_i[i,:,j,trial],s=5,label='$P(Z_{ij}=k|j=%s)$'%j,color=color_b[j])
# Position legend
...
plt.xlabel('$b_{ij}$')
plt.ylabel('$\epsilon_{\dot{x}_{ij}}$')
plt.xlim(np.min(data[0:delta_N,:,0])-0.05,1.5)
...
plt.title('The agent\'s SoA ($Z_{ij}=%s$) over time for trial %s'%(k_agent,trial))
plt.ylabel('$r_i(Z_{ij}=%s)$'%(k_agent))
plt.xlabel('Time /s')
for j in range(n_n):
    plt.scatter(t_p,soa[:,j,trial],s=5,label='$P(Z_{ij}=k|j=%s)$'%j,color=color_b[j])
box = ax.get_position()
...
plt.ylabel('$\sigma^2$')
plt.grid()
# plot covariance of agency cluster in bij direction
...
print('\tof agency over state:',np.mean(r_i[k_agent,:,:,:],axis=(0,2)))
print('\tof no-agency 1 over state:',np.mean(r_i[k_env[0],:,:,:],axis=(0,2)))
print('\tof no-agency 2 over state:',np.mean(r_i[k_env[1],:,:,:],axis=(0,2)))
print('\tof no-agency 3 over state:',np.mean(r_i[k_env[2],:,:,:],axis=(0,2)))
